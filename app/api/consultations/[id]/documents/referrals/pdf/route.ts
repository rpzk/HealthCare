...
export const POST = withDoctorAuth(async (req: NextRequest, { params, user }: { params: Record<string, string>, user: any }) => {
  try {
    ...
    if (consultation.doctorId !== user.id && user.role !== 'ADMIN') {
    ...
    const consultationDateLabel = consultationDate
      ...
      : null
    // Monta HTML do encaminhamento
    let html = `<html><head><meta charset='utf-8'><title>Encaminhamentos</title></head><body>`
    html += `<h1 style='text-align:center'>ENCAMINHAMENTOS</h1>`
    ...
    logger.error({ err }, 'Erro ao gerar PDF de encaminhamentos')
    return NextResponse.json({ error: err.message || 'Erro ao gerar PDF de encaminhamentos' }, { status: 500 })
  }
})
...
export const GET = withDoctorAuth(async (req: NextRequest, { params, user }) => {
  try {
    ...
    const pdf = await signPdfWithGotenberg({ html, filename: `encaminhamentos-${consultationId}.pdf` })
