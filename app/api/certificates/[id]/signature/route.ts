 ...
 *
 * Retorna status e detalhes da assinatura digital de um atestado.
 * Apenas o médico autor ou admins podem consultar.
 */
export const GET = withAuth(async (_req, { params, user }) => {
  try {
    ...
    const withinValidity =
      !!cert?.isActive &&
      new Date(cert.notBefore) <= now &&
      new Date(cert.notAfter) >= now
    ...
    logger.error('[Certificates] Erro ao obter assinatura do atestado:', error)
