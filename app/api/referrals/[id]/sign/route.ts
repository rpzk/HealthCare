...
export const POST = withAuth(async (request: NextRequest, { user, params }) => {
  try {
    ...
    if (referral.doctorId !== user.id) {
      return NextResponse.json(
        { error: 'Apenas o médico autor do encaminhamento pode assinar com seu certificado digital. O certificado é de uso exclusivo do titular.' },
        { status: 403 }
      )
    }
    ...
    try {
      ...
      logger.error('Erro ao assinar encaminhamento:', {
