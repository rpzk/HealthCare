...
import { Loader2, FlaskConical as FlaskIcon, Link as LinkIcon } from 'lucide-react'
...
export default function ExamRequestDetails({ id }: { id: string }) {
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      setPassword('')
      alert('Solicitação de exame assinada com sucesso!')
    } catch (e) {
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold flex items-center gap-2">
            <FlaskIcon className="w-4 h-4" /> Solicitação de Exame #{data.id}
          </h3>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            ...
            onClick={async () => {
              try {
                const res = await fetch(`/api/exam-requests/${id}/signature`)
                const s = await res.json()
                if (!s?.signed || !s?.verificationPageUrl) {
                  alert('Assine a solicitação de exame antes de compartilhar. Quem abrir o link poderá verificar a autenticidade.')
                  return
                }
                const shareUrl = `${window.location.origin}${s.verificationPageUrl}`
                if ((navigator as any)?.share) {
                  await (navigator as any).share({ title: 'Verificação de solicitação de exame', url: shareUrl })
                } else {
          ...
          <Button variant="outline" onClick={() => router.push(`/exams/requests/${id}/edit`)}>Editar</Button>
        </div>
      </div>
        ...
        <CardContent className="p-4 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <p className="text-gray-700">{data.urgency}</p>
            </div>
          </div>
          {data.description && (
            <div>
      ...
      <Dialog open={showPasswordDialog} onOpenChange={setShowPasswordDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Digite a senha do seu certificado A1 para assinar esta solicitação de exame.
            </DialogDescription>
          </DialogHeader>
