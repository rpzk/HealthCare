...
export default function ExamResultDetails({ id }: { id: string }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!res.ok) throw new Error('Falha ao carregar resultado de exame')
        const json = await res.json()
        setData(json)

        const sigRes = await fetch(`/api/exam-results/${id}/signature`)
        if (sigRes.ok) {
        ...
        if (policyRes.ok) {
          ...
          const flag = !!p?.policy?.requireSignature?.examResult
  ...
  const handleSign = async () => {
    ...
    setSigning(true)
    try {
      const res = await fetch(`/api/exam-results/${id}/sign`, {
        method: 'POST',
      ...
      setPassword('')
      alert('Resultado de exame assinado com sucesso!')
    } catch (e) {
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold flex items-center gap-2">
            <FileIcon className="w-4 h-4" /> Resultado de Exame #{data.id}
          </h3>
          <p className="text-sm text-gray-600">Paciente: {data.patient.name}</p>
        </div>
        <div className="space-x-2">
          ...
          <Button
            ...
            onClick={() => {
              ...
              if ((navigator as any)?.share) {
                (navigator as any).share({ title: 'Resultado de Exame', url: shareUrl }).catch(() => {})
              } else {
          ...
          <Button variant="outline" onClick={() => router.push(`/exams/results/${id}/edit`)}>Editar</Button>
        </div>
      </div>
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              <h4 className="font-medium">Tipo de Exame</h4>
              <p className="text-gray-700">{data.examType}</p>
            </div>
            <div>
              <h4 className="font-medium">Data do Exame</h4>
              <p className="text-gray-700">{new Date(data.examDate).toLocaleString('pt-BR')}</p>
            </div>
          </div>
          <div>
            <h4 className="font-medium">Resultados</h4>
            <pre className="text-gray-800 whitespace-pre-wrap bg-muted/30 p-3 rounded-md border border-muted-foreground/20">
              {data.results}
            </pre>
          </div>
        </CardContent>
      ...
      <Dialog open={showPasswordDialog} onOpenChange={setShowPasswordDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Digite a senha do seu certificado A1 para assinar este resultado de exame.
            </DialogDescription>
          </DialogHeader>
